...
export default function MatchingCard({matchData, userId}: MatchingUserProps) {
    ...
    const buttonMatchingAction = async (actionType: string) => {
        const currentMatch = matchBuffer[0]; // The first user in the buffer
    ...
    console.log("MATCHING BUFFER");
    console.log(matchBuffer);
    console.log("END CMATCHINGBUFFER");
    ...
    return (
        <>
            ...
            {/*    {["dislike", "heart", "like"].map((action) => (*/}
            {/*        <button*/}
            {/*            key={action}*/}
            {/*            onClick={() => buttonMatchingAction(action, currentMatch)}*/}
